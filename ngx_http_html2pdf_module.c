...
typedef struct {
    ...
    u_char *last;
    ngx_uint_t phase;
} ngx_http_html2pdf_ctx_t;

ngx_module_t ngx_http_html2pdf_module;
...
static ngx_command_t ngx_http_html2pdf_commands[] = {
  ...
  { .name = ngx_string("html2pdf_buffer_size"),
    ...
    .conf = NGX_HTTP_LOC_CONF_OFFSET,
    .offset = offsetof(ngx_http_html2pdf_loc_conf_t, buffer_size),
    .post = NULL },
    ngx_null_command
};

static void *ngx_http_html2pdf_create_loc_conf(ngx_conf_t *cf) {
    ngx_http_html2pdf_loc_conf_t *conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_html2pdf_loc_conf_t));
...
static char *ngx_http_html2pdf_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child) {
...
static ngx_int_t ngx_http_html2pdf_header_filter(ngx_http_request_t *r) {
    ...
    ngx_http_html2pdf_loc_conf_t *conf = ngx_http_get_module_loc_conf(r, ngx_http_html2pdf_module);
    if (!conf->enable) goto ngx_http_next_header_filter;
    ctx = ngx_pcalloc(r->pool, sizeof(ngx_http_html2pdf_ctx_t));
...
static ngx_int_t ngx_http_html2pdf_body_filter(ngx_http_request_t *r, ngx_chain_t *in) {
    ...
    switch (ctx->phase) {
        ...
        case NGX_HTTP_HTML_READ: {
            switch (ngx_http_html2pdf_html_read(r, in)) {
        ...
        case NGX_HTTP_HTML_PROCESS: {
            ngx_chain_t out = {.buf = ngx_http_html2pdf_html_process(r), .next = NULL};
            if (!out.buf) goto ngx_http_filter_finalize_request;
            ctx->phase = NGX_HTTP_HTML_PASS;
            return ngx_http_html2pdf_html_send(r, &out);
        }
...
ngx_http_filter_finalize_request:
    return ngx_http_filter_finalize_request(r, &ngx_http_html2pdf_module, NGX_HTTP_UNSUPPORTED_MEDIA_TYPE);
}

static ngx_int_t ngx_http_html2pdf_postconfiguration(ngx_conf_t *cf) {
    ngx_http_next_header_filter = ngx_http_top_header_filter;
    ngx_http_top_header_filter = ngx_http_html2pdf_header_filter;
    ngx_http_next_body_filter = ngx_http_top_body_filter;
    ngx_http_top_body_filter = ngx_http_html2pdf_body_filter;
    return NGX_OK;
}

static ngx_http_module_t ngx_http_html2pdf_module_ctx = {
    .preconfiguration = NULL,
    .postconfiguration = ngx_http_html2pdf_postconfiguration,
    .create_main_conf = NULL,
    .init_main_conf = NULL,
    .create_srv_conf = NULL,
    .merge_srv_conf = NULL,
    .create_loc_conf = ngx_http_html2pdf_create_loc_conf,
    .merge_loc_conf = ngx_http_html2pdf_merge_loc_conf
};

ngx_module_t ngx_http_html2pdf_module = {
    NGX_MODULE_V1,
    .ctx = &ngx_http_html2pdf_module_ctx,
    .commands = ngx_http_html2pdf_commands,
